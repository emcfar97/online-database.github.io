...
function update_gallery(data) {
    ...
    data.forEach( async (row) => {
        ...
        thumbnail = `<img class="thumbnail" src=${get_path(row[1])}>`
        ...
        rows += (
            `<button id="${i++}" class="row" onclick="select_row(this, event)">${thumbnail}${filename}${details}</button>`
            );
        // rows += (
